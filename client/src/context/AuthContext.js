...
export const AuthProvider = ({ children }) => {
    ...
    const register = async (email, password, role, first_name, date_of_birth) => {
        const res = await axios.post(`${API_URL}/api/auth/register`, {
            email, password, role, first_name, date_of_birth
        });
        localStorage.setItem('token', res.data.token);
        setUser(res.data.user);
        return res.data;
    ...
    return (
        <AuthContext.Provider value={{ user, checkUser, login, loginWithTelegram, register, logout, loading }}>
            {!loading && children}
