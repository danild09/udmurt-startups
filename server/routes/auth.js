  ...
  try {
    ...
    db.get('SELECT * FROM users WHERE email = ?', [email], (err, user) => {
      ...
      db.run(
        ...
        [email, password_hash, assignedRole, first_name || '', last_name || ''],
        function (err) {
          ...
          res.json({
            token,
            user: { id: newUserId, email, role: assignedRole, first_name, last_name }
          });
        }
      );
    });
...
router.get('/me', authMiddleware, (req, res) => {
  db.get('SELECT id, email, telegram_id, username, first_name, last_name, role, created_at FROM users WHERE id = ?', [req.user.id], (err, user) => {
